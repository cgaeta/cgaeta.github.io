...
$(document).ready(function(e) {
  ...
  function animate() {
    ...
    //frame1
    fadein([logoMM, logoHL, txtLegal], .1);
    slidein(txtHeadline1, .6);

    //frame2
    TweenLite.to(txtHeadline2_1, .5, {opacity: 1, x: 160, delay: f2});
    TweenLite.to(txtHeadline2_2, .5, {opacity: 1, scale: 1, delay: f2+2});
    TweenLite.to(txtHeadline2_3, .5, {opacity: 1, x: -160, delay: f2+3});
...
function slidein(el, d) { TweenLite.to(el, .5, {opacity: 1, x: -160, delay: d}); }

function fadein(el, d) { TweenLite.to(el, .5, {opacity: 1, delay: d}); }
