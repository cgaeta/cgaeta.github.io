...
// try to make an AJAZ request for Twitter feed
	...
	req.onreadystatechange = function(){
		...
		if(req.readyState === 4)
			if(req.status >= 200 && req.status < 400){
				...
				tweetCont.className = "tweetCont";
				
				// add anchor tags around links
				var linksReg = /(https\:(?:&#x2F;|\/){2}t\.co(?:\/|&#x2F;)?\w+)/g;
				var replaced = req.responseText.replace(linksReg, "<a rel='noreferrer noopener' href='$1' target='_blank'>$1</a>");
				
				// add anchor tags around usernames
				var usersReg = /(@\w+)/g;
				replaced = replaced.replace(usersReg, "<a rel='noreferrer noopener' href='https://twitter.com/$1' target='_blank'>$1</a>");
				
				console.log(replaced);
				
				tweetCont.innerHTML = replaced;
